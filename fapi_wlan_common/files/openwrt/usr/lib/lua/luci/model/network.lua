...
function wifinet.active_bssid(self)
	...
	local m = _stror(self.iwdata.mode, self.iwinfo.mode) or "ap"
	if m == "wds" or m == "ap"
	-- for WDS/AP return interface mac add 
	-- bssid is used for WDS peer in WDS mode and is not applicable for ap
	then 
		return _stror(self.iwdata.macaddr, self.iwinfo.bssid) or "00:00:00:00:00:00"
	else
