...
if found_sta then
	...
	if iw.hwmodelist == nil then
		function ch.formvalue(self, section)
			return {
				m:formvalue(self:cbid(section) .. ".band") or "",
...
if hwtype == "intel_ap" then
	if not st.ifname:match("^wlan[0-9].network1$") then
		disabled = s:taboption("general", Flag, "disabled", translate("Enable Interface"))
		disabled.rmempty = false

		function disabled.cfgvalue(self, section)
			local disabled = Value.cfgvalue(self, section)
			if disabled == "1"
			then
				disabled = "0"
			else
...
if hwtype == "intel_ap" then
	bssid = s:taboption("general", Value, "bssid", translate("<abbr title=\"WDS Peer MAC Address\">WDS MAC Address</abbr>"))
	macaddr = s:taboption("general", Value, "macaddr", translate("<abbr title=\"Basic Service Set Identifier\">BSSID</abbr>"))
else
...
if hwtype ~= "intel_ap" then
	...
	function network.write(self, section, value)
		...
		if value == '-' or value == "" then
			m.uci:set("wireless", section, "network", "")
		else
			m.uci:set("wireless", section, "network", value)
		end
	end
end
-------------------- MAC80211 Interface ----------------------
...
if hwtype == "intel_ap" then
	...
	function mode.write(self, section, value)
		if value == "wds" then
			...
			m.uci:set("wireless", section, "wds", 0)
		end
	end
	...
	local iw = luci.sys.wifi.getiwinfo(st.ifname)
	local hw_list = iw.hwmodelist
	if hw_list~=nil and not hw_list.b then
		s:taboption("advanced", Flag, "doth", "802.11h")
	end
	...
	ml:depends({macpolicy="allow"})
	ml:depends({macpolicy="deny"})
	nt.mac_hints_no_dns_resolution(function(mac, name) ml:value(mac, "%s (%s)" %{ mac, name }) end)
...
if hwtype == "intel_ap" then
	server = s:taboption("encryption", Value, "server", translate("Radius-Authentication-Server"))
...
if hwtype == "intel_ap" then
	port = s:taboption("encryption", Value, "port", translate("Radius-Authentication-Port"), translatef("Default %d", 1812))
...
if hwtype == "intel_ap" then
	--For enterprise security key field is used as radius secret
	radius_key = s:taboption("encryption", Value, "_radius_key", translate("Radius secret"))
	radius_key:depends("encryption", "wpa+tkip")
	radius_key:depends("encryption", "wpa2+aes")
	radius_key:depends("encryption", "wpa-mixed+tkip+aes")
	radius_key.password = true
	radius_key.rmempty = true
	--For enterprise mode 'key' is used for radius secret
	--radius_secret is just a dummy and will not be set in conf file
	radius_key.cfgvalue = function(self, section, value)
	...
	radius_key.write = function(self, section, value)
...
if hwtype == "intel_ap" then
	wpakey:depends("encryption", "psk+tkip")
	wpakey:depends("encryption", "psk2+aes")
	wpakey:depends("encryption", "psk-mixed+tkip+aes")
end
...
if hwtype ~= "intel_ap" then
	...
	slot=1
	wepkey = s:taboption("encryption", Value, "key" .. slot, translatef("WEP Key", slot))
