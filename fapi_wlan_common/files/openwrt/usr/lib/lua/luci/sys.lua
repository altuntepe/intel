...
--          [ "mac", "name/ip" ]
--          Function does not use DNS resolution in order to prevent a
--          web page from getting 'stuck' in case DNS resolution takes
--          time
 
function net.mac_hints_no_dns_resolution(callback)
	if callback then
		_nethints(1, function(mac, v4, v6, name)
			if mac and mac ~= "" then
				if (name and name ~= mac) or v4 then
